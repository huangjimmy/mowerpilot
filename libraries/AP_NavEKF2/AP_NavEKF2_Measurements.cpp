...
void NavEKF2_core::readMagData(MagnetoData magnetoData)
{
    ...
    if (use_compass() && magnetoData.compass_last_update_usec - lastMagUpdate_us > 70000) {
        ...
        if (magTimeout && (maxCount > 1) && !onGround && imuSampleTime_ms - ekfStartTime_ms > 30000) {
            ...
            for (uint8_t i=1; i<maxCount; i++) {
                ...
                if (magnetoData.compassData[tempIndex].use_for_yaw && tempIndex != magSelectIndex) {
        ...
        Vector3f nowMagOffsets = magnetoData.compassData[magSelectIndex].compass_offset;
        ...
        lastMagUpdate_us = magnetoData.compassData[magSelectIndex].last_update_usec;
        ...
        magDataNew.mag = magnetoData.compassData[magSelectIndex].compass_field * 0.001f;

        // check for consistent data between magnetometers
        consistentMagData = magnetoData.compassData[magSelectIndex].compass_consistent;
 ...
 *  errors.
 */
void NavEKF2_core::readIMUData(ftype ins_loop_delta_t, uint32_t hal_millis, Vector3f ins_accelPosOffset, Vector3f imuDataNew_delAng, float imuDataNew_delAngDT)
{

    // average IMU sampling rate
    dtIMUavg = ins_loop_delta_t;//ins.get_loop_delta_t();

    // the imu sample time is used as a common time reference throughout the filter
    imuSampleTime_ms = hal_millis;//AP_HAL::millis(); //TODO: need to get imuSampleTime_ms
...
//    }  //TODO: need to get accelPosOffset
    accelPosOffset = ins_accelPosOffset;
...
//    } //TODO: need to get imuDataNew.delAng and imuDataNew.delAngDT
    imuDataNew.delAng = imuDataNew_delAng;
    imuDataNew.delAngDT = imuDataNew_delAngDT;
...
void NavEKF2_core::readGpsData(GpsData gpsData)
{
    // check for new GPS data
    // do not accept data at a faster rate than 14Hz to avoid overflowing the FIFO buffer
//    const AP_GPS &gps = AP::gps();
    if (gpsData.gps_last_message_time_ms - lastTimeGpsReceived_ms > 70) {
        if (gpsData.gps_status >= GPS_OK_FIX_3D) {
            ...
            lastTimeGpsReceived_ms = gpsData.gps_last_message_time_ms;


            // estimate when the GPS fix was valid, allowing for GPS processing and other delays
            // ideally we should be using a timing signal from the GPS receiver to set this time
            float gps_delay = gpsData.gps_delay;
//            gps.get_lag(gps_delay); // ignore the return value
            ...
            gpsDataNew.sensor_idx = gpsData.gps_primary_sensor;

            // read the NED velocity from the GPS
            gpsDataNew.vel = gpsData.gps_velocity;

            // Use the speed and position accuracy from the GPS if available, otherwise set it to zero.
            // Apply a decaying envelope filter with a 5 second time constant to the raw accuracy data
            float alpha = constrain_float(0.0002f * (lastTimeGpsReceived_ms - secondLastGpsTime_ms),0.0f,1.0f);
            gpsSpdAccuracy *= (1.0f - alpha);
            float gpsSpdAccRaw = gpsData.gps_speed_accuracy;
...
//            }
            ...
            float gpsPosAccRaw = gpsData.gps_horizontal_accuracy;
            if (!gpsData.gps_horizontal_accuracy_ok) {
                gpsPosAccuracy = 0.0f;
            } else {
            ...
            float gpsHgtAccRaw = gpsData.gps_vertical_accuracy;
            if (!gpsData.gps_vertical_accuracy_ok) {
                gpsHgtAccuracy = 0.0f;
            } else {
            ...
            if (gpsData.gps_num_sats >= 6 && (PV_AidingMode == AID_ABSOLUTE)) {
                gpsNoiseScaler = 1.0f;
            } else if (gpsData.gps_num_sats == 5 && (PV_AidingMode == AID_ABSOLUTE)) {
                gpsNoiseScaler = 1.4f;
            ...
            if (gpsData.gps_have_vertical_velocity && frontend->_fusionModeGPS == 0 && !frontend->inhibitGpsVertVelUse) {
                useGpsVertVel = true;
            } else {
                useGpsVertVel = false;
            }
            ...
            const struct Location &gpsloc = gpsData.gps_location;
...
void NavEKF2_core::readBaroData(BaroData baroData)
{
    // check to see if baro measurement has changed so we know if a new measurement has arrived
    // do not accept data at a faster rate than 14Hz to avoid overflowing the FIFO buffer

    if (baroData.baro_last_update - lastBaroReceived_ms > 70) {
        frontend->logging.log_baro = true;

        baroDataNew.hgt = baroData.baro_altitude;
        ...
        lastBaroReceived_ms = baroData.baro_last_update;
...
void NavEKF2_core::readAirSpdData(AirSpdData aspeed)
{
    // if airspeed reading is valid and is set by the user to be used and has been updated then
    // we take a new reading, convert from EAS to TAS and set the flag letting other functions
    // know a new measurement is available
    if (aspeed.airSpd_use &&
            aspeed.airSpd_last_update_ms != timeTasReceived_ms) {
        tasDataNew.tas = aspeed.airSpd_airspeed * aspeed.airSpd_EAS2TAS;
        timeTasReceived_ms = aspeed.airSpd_last_update_ms;
...
void NavEKF2_core::readRngBcnData(RngBcnData rngBcnData)
{
    // get the location of the beacon data

    // exit immediately if no beacon object
    if (rngBcnData.has_rngBcn == false) {
        return;
    }

    // get the number of beacons in use
    N_beacons = rngBcnData.beacon_count;
    ...
    while (!newDataToPush && numRngBcnsChecked < N_beacons) {
        ...
        if (rngBcnData.beaconData[index].beacon_healthy &&
                rngBcnData.beaconData[index].beacon_last_update_ms!= lastTimeRngBcn_ms[index])
        {
            // set the timestamp, correcting for measurement delay and average intersampling delay due to the filter update rate
            lastTimeRngBcn_ms[index] = rngBcnData.beaconData[index].beacon_last_update_ms;
            ...
            rngBcnDataNew.rng = rngBcnData.beaconData[index].beacon_distance;

            // set the beacon position
            rngBcnDataNew.beacon_posNED = rngBcnData.beaconData[index].beacon_position;
    ...
    // Check if the beacon system has returned a 3D fix //TODO: assume always a 3D fix
    if (rngBcnData.get_vehicle_position_ned) {
        beaconVehiclePosNED = rngBcnData.beaconVehiclePosNED;
        beaconVehiclePosErr = rngBcnData.beaconVehiclePosErr;
    ...
    if (imuSampleTime_ms - rngBcnLast3DmeasTime_ms < 250 && beaconVehiclePosErr < 1.0f && rngBcnAlignmentCompleted) {
        ...
        if (posDiffSq < 9.0f*posDiffVar) {
            ...
            if (!validOrigin && PV_AidingMode != AID_ABSOLUTE) {
                ...
                Location origin_loc;
                //TODO: assume no origin loc
                if (rngBcnData.beacon_get_origin) {
                    origin_loc = rngBcnData.beacon_origin;
